...
class ParentheticalProcessor:
    __MODIFIABLE = r'(?:quotations?|quotes?|(?:quotation )?marks?|ellips.s|cites?|citations?|emphas.s|italics?|footnotes?|alterations?|punctuation|modifications?|brackets?|bracketed material|formatting)'
    __MODIFABLE_TYPE = r'(?:internal|former|latter|first|second|last|some|further|certain)'
    ...
    __FULL_OPINION_DESCRIPTOR = f'(?:{__OPINION_TYPES}(?: {__OPINION_TYPE_MODIFICATION})?)'
    ...
    PARENTHETICAL_REGEX_BLACKLIST_RULES = [
        ...
        r'dictum|dicta',
        r'simplified|cleaned up',
        r'same|similar|contra',
        f'{__DOCUMENT_TYPES} below',  # case below
        f'{__AGGREGATOR_TYPES} cases',  # collecting cases, reviewing cases
    ...
    PARENTHETICAL_BLACKLIST_REGEX = re.compile(__PREFIX + r'|'.join(
        map(lambda reg: f'(?:{reg})', PARENTHETICAL_REGEX_BLACKLIST_RULES)) + __SUFFIX, re.IGNORECASE)
