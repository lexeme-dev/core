    ...
    def case_recall(
        ...
        for c in cases:
            ...
            for i in range(num_trials):
                ...
                out_neighbors[removed_idx], out_neighbors[-1] = (
                    out_neighbors[-1],
                ...
                same_court = (
                    (
                        self.citation_network.network_edge_list.node_metadata[
                            removed
                        ].court,
                    )
                    if same_court
                ...
                recommendations = list(
                    self.recommendation.recommendations(
                        frozenset(out_neighbors),
                        20,
                        courts=frozenset(court + same_court),
                        ignore_opinion_ids=frozenset([c]),
                        strategy=strategy,
                        before_year=md.year,
                    ).keys()
                )
