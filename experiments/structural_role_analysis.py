...
def get_role_summaries(role_dict: RoleDict) -> List[RoleSummary]:
    ...
    for role_id, resource_ids in role_dict.items():
        ...
        average_in_degree = Citation.select().where(
            Citation.cited_opinion_id << resource_ids
        ).count() / len(resource_ids)
        average_out_degree = Citation.select().where(
            Citation.citing_opinion_id << resource_ids
        ).count() / len(resource_ids)
