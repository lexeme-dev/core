...
def connect_to_database():
    load_dotenv()
    deebee_host, deebee_name, deebee_username, deebee_password, deebee_port = (os.getenv('DB_HOST'), os.getenv('DB_NAME'),
                                                           os.getenv('DB_USERNAME'), os.getenv('DB_PASSWORD'),
                                                           os.getenv('DB_PORT'))
    return PostgresqlDatabase(deebee_name, user=deebee_username, password=deebee_password, host=deebee_host, port=deebee_port)
...
def model_list_to_dicts(peewee_models: List[Model], **kwargs):
    from deebee.peewee.models import DEFAULT_SERIALIZATION_ARGS
