...
top_opinions = [opinion_id for opinion_id, centrality_score in sorted(centrality.items(), key=lambda item: item[1], reverse=True)][:25]
output_str = ""
name_map = {}
for i, opinion_id in enumerate(top_opinions):
    try:
        ...
        name_arr[0] = " ".join(name_arr[0].split(" ")[:2]) if len(name_arr[0]) > 25 else name_arr[0]
        name_arr[1] = " ".join(name_arr[1].split(" ")[:2]) if len(name_arr[1]) > 25 else name_arr[1]
        name_arr.insert(1, " v. \n")
