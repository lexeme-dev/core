...
class ClFileDownloader:
    ...
    def download(self):
        for jur in self.jurisdictions:
            threads = [
                threading.Thread(target=self.get_data_folder, args=(CLUSTER_PATH, jur)),
    ...
    def get_data_folder(self, resource_type: str, jurisdiction: str):
        Logger.info(
            f"Downloading {resource_type} data for jurisdiction {jurisdiction}..."
        )
        tar_file_bytes = urllib.request.urlopen(
            self.__get_download_url(resource_type, jurisdiction)
        ).read()
        Logger.info(
            f"Completed {resource_type} data download for jurisdiction {jurisdiction}, extracting..."
        )
        ...
        Logger.info(
            f"Completed extraction of {resource_type} data for jurisdiction {jurisdiction}..."
        )
        return folder_path
    ...
    def get_citation_csv(self):
        ...
        tar_file_bytes = urllib.request.urlopen(
            f"{BASE_URL}/{REMOTE_CITATIONS_PATH}"
        ).read()
