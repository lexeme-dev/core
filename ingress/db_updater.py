...
class DbUpdater:
                ...
                if filename.endswith(".json"):
                    ...
                    with open(file_path, "rb") as json_file:
                        ...
                        if (
                            ...
                            and cluster_checksum_dict.get(resource_id) == file_checksum
                        ):
                            continue
                        ...
                        case_name = (
                            cluster_data["case_name"]
                            or cluster_data["case_name_full"]
                            or cluster_data["case_name_short"]
                        )
                        new_record = dict(
    ...
    def process_opinion_data(
        self, dir_path: str, include_text: bool, jurisdiction: str
    ):
            ...
            try:
                ...
                if filename.endswith(".json"):
                    ...
                    with open(file_path, "rb") as json_file:
                        ...
                        if (
                            not self.force_update
                            and opinion_checksum_dict.get(resource_id) == file_checksum
                        ):
                            continue
        ...
        self.__batch_query(
            lambda records: self.__upsert_opinions_to_db(records, include_text),
            opinion_records,
    ...
    def process_citation_data(self, citations_file):
        ...
        with open(citations_file) as csv_file:
            ...
            for row in csv_reader:
                try:
                    citing_opinion, cited_opinion, depth = (
                        int(row[0]),
                        int(row[1]),
                        int(row[2]),
                    )
                    if (citing_opinion, cited_opinion, depth) in citation_set:
                        continue
                    if (
                        citing_opinion in opinion_checksum_dict
                        and cited_opinion in opinion_checksum_dict
                    ):
                        new_record = dict(
        ...
        Logger.info(
            f"Finished reading CL citation data, upserting {len(citation_records)} records..."
        )
        self.__batch_query(
            self.__upsert_citations_to_db, citation_records, batch_size=100_000
        )
        self.session.commit()
    ...
    def __batch_query(
        self, query_func: Callable, records: List[dict], batch_size=DEFAULT_BATCH_SIZE
    ):
    ...
    def __upsert_clusters_to_db(self, clusters: List[dict]):
        ...
        query = query.on_conflict_do_update(
            index_elements=[Cluster.resource_id],
            set_=dict(
                case_name=query.excluded.case_name,
    ...
    def __upsert_opinions_to_db(self, clusters: List[dict], include_text: bool):
        ...
        query = query.on_conflict_do_update(
            index_elements=[Opinion.resource_id], set_=conflict_update_dict
        )
        self.session.execute(query)
    ...
    def __upsert_citations_to_db(self, citations: List[dict]):
        ...
        query = query.on_conflict_do_update(
            index_elements=[Citation.citing_opinion_id, Citation.cited_opinion_id],
    ...
    def __get_cluster_checksum_dict(self) -> Dict[int, str]:
        res = self.session.execute(
            select(Cluster.resource_id, Cluster.courtlistener_json_checksum)
        ).all()
        return {resource_id: checksum for resource_id, checksum in res}

    def __get_opinion_checksum_dict(self) -> Dict[int, str]:
        res = self.session.execute(
            select(Opinion.resource_id, Opinion.courtlistener_json_checksum)
        ).all()
        return {resource_id: checksum for resource_id, checksum in res}
